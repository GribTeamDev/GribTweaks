package com.rumaruka.gribtweaks.compat.jei;

import com.rumaruka.gribtweaks.common.recipe.PrimitiveBrushRecipe;
import com.rumaruka.gribtweaks.compat.jei.category.BrushCategory;
...
public class GribTweaksJEI implements IModPlugin {
    ...
    private void loadCategories() {
        ...
        CreateRecipeCategory<?>
                ...
                polishing = builder(PrimitiveBrushRecipe.class)
                .addTypedRecipes(GTRecipeType.PRIMITIVE_BRUSH)
                .catalyst(GTItems.primitive::get)
                .itemIcon(GTItems.primitive.get())
                .emptyBackground(177, 55)
                .build("primitive_brush", BrushCategory::new);
    ...
    private class CategoryBuilder<T extends Recipe<?>> {
        ...
        public CategoryBuilder<T> addTypedRecipes(Supplier<RecipeType<? extends T>> recipeType) {
            return addRecipeListConsumer(recipes -> GribTweaksJEI.<T>consumeTypedRecipes(recipes::add, recipeType.get()));
        }

        public CategoryBuilder<T> addTypedRecipes(Supplier<RecipeType<? extends T>> recipeType, Function<Recipe<?>, T> converter) {
            return addRecipeListConsumer(recipes -> GribTweaksJEI.<T>consumeTypedRecipes(recipe -> recipes.add(converter.apply(recipe)), recipeType.get()));
        }

        public CategoryBuilder<T> addTypedRecipesIf(Supplier<RecipeType<? extends T>> recipeType, Predicate<Recipe<?>> pred) {
            return addRecipeListConsumer(recipes -> GribTweaksJEI.<T>consumeTypedRecipes(recipe -> {
                if (pred.test(recipe)) {
                    recipes.add(recipe);
                }
            }, recipeType.get()));
        ...
        public CategoryBuilder<T> addTypedRecipesExcluding(Supplier<RecipeType<? extends T>> recipeType, Supplier<RecipeType<? extends T>> excluded) {
            return addRecipeListConsumer(recipes -> {
                ...
                GribTweaksJEI.<T>consumeTypedRecipes(recipe -> {
        ...
        public CreateRecipeCategory<T> build(String name, CreateRecipeCategory.Factory<T> factory) {
            ...
            CreateRecipeCategory.Info<T> info = new CreateRecipeCategory.Info<>(new mezz.jei.api.recipe.RecipeType<>(rl(name), recipeClass), Lang.translateDirect("recipe." + name), background, icon, recipesSupplier, catalysts);
