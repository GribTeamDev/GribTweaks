...
public class ArchaeologicalGeneratorRenderer implements BlockEntityRenderer<ArchaeologicalGeneratorBlockEntity> {
    public static final ResourceLocation BEAM_LOCATION = new ResourceLocation("textures/entity/beacon_beam.png");
    public static final int MAX_RENDER_Y = 1024;

    public ArchaeologicalGeneratorRenderer(BlockEntityRendererProvider.Context pContext) {
    }

    public void render(ArchaeologicalGeneratorBlockEntity pBlockEntity, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBufferSource, int pPackedLight, int pPackedOverlay) {
        long i = pBlockEntity.getLevel().getGameTime();
        List<ArchaeologicalGeneratorBlockEntity.BeaconBeamSection> list = pBlockEntity.getBeamSections();
        int j = 0;

        for(int k = 0; k < list.size(); ++k) {
            ArchaeologicalGeneratorBlockEntity.BeaconBeamSection beaconblockentity$beaconbeamsection = list.get(k);
    ...
    public boolean shouldRenderOffScreen(ArchaeologicalGeneratorBlockEntity pBlockEntity) {
        return true;
    }
    ...
    public boolean shouldRender(ArchaeologicalGeneratorBlockEntity pBlockEntity, Vec3 pCameraPos) {
