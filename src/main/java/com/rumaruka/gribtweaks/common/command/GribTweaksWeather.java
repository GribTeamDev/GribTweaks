...
public class GribTweaksWeather {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("gribweather").requires((p) -> p.hasPermission(1)).executes(c -> sendUsage(c.getSource()))
                .then(Commands.literal("clear").executes(c -> execute(c.getSource(), isSandstorm, 0)).then(Commands.argument("time", IntegerArgumentType.integer(-1)).executes(c -> execute(c.getSource(), false, IntegerArgumentType.getInteger(c, "time")))))
                .then(Commands.literal("sandstorm").executes(c -> execute(c.getSource(), isSandstorm, 0)).then(Commands.argument("time", IntegerArgumentType.integer(-1)).executes(c -> execute(c.getSource(), true, IntegerArgumentType.getInteger(c, "time"))))));
    }
